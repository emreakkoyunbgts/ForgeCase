...
from common.contract import client_label, has_outcomes
...
# CF-119: HTTP-only cutover — fetch the corpus from Vault, not a local file.
try:
    corpus_response = call_service("GET", ALL_SERVICES["vault"] + "/engagements")
    corpus = corpus_response.json()["items"]
except Exception as e:
    st.error(f"Could not load engagements from Vault: {e}")
    st.stop()
...
if saved is not None and "case_study" in saved:
    ...
    with st.spinner("Generating..."):
        try:
            sections_response = call_service(
                "POST", ALL_SERVICES["generator"] + "/generator/ai/eng",
                json=record,
                timeout=60,
            )
...
if case_study is not None:
    ...
    try:
        verify_response = call_service(
            "POST", ALL_SERVICES["verifier"] + f"/verify/{engagement_id}",
            json={"record": record, "mcs": case_study},
        )
